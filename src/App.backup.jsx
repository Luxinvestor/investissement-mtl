...
export default function App() {
  ...
  const comparaison = [
    ...
    },
    {
      ...
      value: applyReel(loc.beneficeNet),
      full: 'Je loue mon bien (locatif pur)',
      desc: `Vous achetez un bien que vous louez entièrement — vous habitez ailleurs. Bénéfice net à la revente incluant le cashflow cumulé, après impôt sur le gain en capital (50 % d'inclusion).`
    },
    {
      name: 'Je reste loc. + bourse',
      value: applyReel(bourse.beneficeNet),
      full: 'Je reste locataire et investis en bourse',
      desc: `Vous restez locataire (c'est vous le locataire) et investissez l'équivalent de la mise de fonds + économies annuelles en CELI, REER et compte non-enregistré. Bénéfice net = portefeuille final après impôt moins capital total investi.`
    },
  ];
  ...
  const timeSeriesData = rp.timeseries.map((r, i) => ({
    ...
    'Je reste loc. + bourse': applyReel(bourse.timeseries[i].porteTotal),
  }));
  ...
  return (
    <div className="min-h-screen bg-stone-50" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Display", "Segoe UI", Helvetica, Arial, sans-serif' }}>
      {/* ===== Header ===== */}
      <header className="bg-white border-b border-stone-200">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-4 md:py-5">
          <div className="flex justify-between items-center gap-4 flex-wrap">
            <div>
              <div className="text-xs uppercase tracking-widest text-stone-400 mb-0.5">Simulateur · Île de Montréal · 2026</div>
            ...
            <div className="flex gap-2 flex-wrap items-center">
              ...
              <label className="hidden sm:flex items-center gap-2 text-xs bg-stone-100 border border-stone-200 rounded-lg px-3 py-1.5 cursor-pointer">
                ...
                <Tooltip2 text="Affiche tous les montants en dollars de 2026. Ex : 500 000 $ en 2051 vaut ~270 000 $ d'aujourd'hui à cause de l'inflation. Coché = plus facile à comparer les stratégies." below>
      ...
      {/* ===== Bandeau mode avancé ===== */}
      {mode === 'avance' && (
        <div className="bg-slate-800 text-slate-100 text-xs px-4 md:px-6 py-2 flex items-center gap-3 flex-wrap">
          <span className="bg-slate-600 rounded px-2 py-0.5 font-semibold tracking-wide uppercase">Mode Avancé</span>
          <span className="text-slate-300">Sensibilité · Stress tests · Décaissement retraite · Paramètres avancés</span>
          <button onClick={() => setMode('debutant')} className="ml-auto text-slate-400 hover:text-white underline">Retour au mode débutant</button>
        </div>
      )}
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-4">
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-5">
            ...
            <MiniSlider
              ...
              tooltip="Taux annuel de votre prêt hypothécaire. Au Canada, fixé pour 5 ans typiquement, puis renouvelé. Actuellement ~4,5-5,5 % pour un terme fixe 5 ans."
            />
            <MiniSlider
              label="Rendement bourse"
              value={h.rendement} onChange={update('rendement')} min={0.03} max={0.12} step={0.0025} format="pct"
              tooltip="Rendement annuel moyen d'un portefeuille d'ETF (ex: S&P 500). Historique sur 30 ans : ~9-10 % nominal. On utilise 8 % par prudence. Rendement réel (après inflation) ≈ 5,5 %."
            />
            <MiniSlider
          ...
          <div className="mt-2 text-xs text-stone-400">
            ...
            {' · '}Versement/mois : <span className="text-stone-600 font-medium">{fmtMoney(rp.versementMens)}</span>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 pt-4 hidden sm:block">
        <div className="flex items-end border-b border-stone-200">
          ...
          {/* Onglets centraux */}
          {[
            { id: 'profil', label: 'Mes infos', mode: 'all' },
            { id: 'rp', label: 'Résidence principale', mode: 'all' },
            { id: 'duplex', label: 'Multiplex', mode: 'all' },
            { id: 'loc', label: 'Je loue mon bien', mode: 'all' },
            { id: 'bourse', label: 'Bourse', mode: 'all' },
            { id: 'nonfin', label: 'Non-financier', mode: 'all' },
            { id: 'sensibilite', label: 'Sensibilité', mode: 'avance' },
            { id: 'stress', label: 'Stress tests', mode: 'avance' },
            { id: 'retraite', label: 'Retraite', mode: 'avance' },
          ].filter(t => t.mode === 'all' || mode === 'avance').map(t => {
            const accentColor = mode === 'avance' ? 'border-slate-600' : 'border-stone-700';
            return (
              <button
                key={t.id}
                onClick={() => setActiveTab(t.id)}
                className={`px-3 py-2 text-sm font-medium transition-colors whitespace-nowrap flex-shrink-0 ${
                  activeTab === t.id ? `border-b-2 ${accentColor} text-stone-900 -mb-px` : 'text-stone-500 hover:text-stone-700'
                }`}
              >{t.label}</button>
            );
          })}
          ...
          <div className="bg-white w-full max-w-sm ml-auto h-full overflow-y-auto shadow-2xl" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-4 space-y-1">
              {[
                ...
                { id: 'duplex', label: 'Multiplex' },
                { id: 'loc', label: 'Je loue mon bien' },
                { id: 'bourse', label: 'Locataire + Bourse' },
                { id: 'nonfin', label: 'Non-financier' },
                ...(mode === 'avance' ? [
                  { id: 'sensibilite', label: 'Sensibilité' },
                  { id: 'stress', label: 'Stress tests' },
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-6 py-6">
        ...
        {activeTab === 'comparison' && (
          <div className="space-y-5">
            ...
            <details className="bg-emerald-50 border border-emerald-200 rounded-lg overflow-hidden">
              <summary className="px-4 py-3 cursor-pointer text-sm font-semibold text-emerald-900 hover:bg-emerald-100 transition-colors flex items-center justify-between">
                <span className="flex items-center gap-2"><IconInfo size={14} /> À propos de ces résultats</span>
                <IconChevron size={13} />
              </summary>
              <div className="px-4 pb-4 pt-2 text-xs text-emerald-900 space-y-2 border-t border-emerald-200">
                <p>Ce tableau compare le <strong>bénéfice net après {h.horizon} ans</strong> pour 4 stratégies avec le même capital de départ. Pour les 3 stratégies immobilières, le bénéfice est calculé en supposant la <strong>vente du bien à la fin de la période</strong>.</p>
                <p>Tous les montants sont <strong>après impôts</strong> : les scénarios immobiliers tiennent compte de l'impôt sur le gain en capital (sauf résidence principale, totalement exemptée), et la bourse tient compte de l'impôt sur les retraits REER au TMI de retraite.</p>
                <p>Le graphique d'évolution montre la valeur nette <strong>si vous vendiez le bien cette année-là</strong> — frais de commission déduits.</p>
              </div>
            </details>
            {/* Hypothèses du modèle — cliquables */}
            <div className="bg-stone-50 border border-stone-200 rounded-lg px-4 py-3">
              <div className="text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">Hypothèses du modèle</div>
              <div className="flex flex-wrap gap-2">
                {[
                  { label: 'Île de Montréal', desc: 'Données de marché spécifiques à l\'Île de Montréal (prix médian, loyers, appréciation)', tab: null },
                  { label: 'Fiscalité Québec 2026', desc: 'Paliers d\'imposition combinés Québec + fédéral 2026. TMI de 26 % à 53 %. Source : CFFP / Revenu Québec.', tab: 'profil' },
                  { label: `Appréciation immo ${(h.apprec * 100).toFixed(1)} %/an`, desc: 'Taux annuel de hausse de la valeur des propriétés. Moyenne historique Montréal ~3-4 %/an. Ajustable dans les paramètres.', tab: 'params' },
                  { label: `Rendement bourse ${(h.rendement * 100).toFixed(1)} %/an`, desc: 'Rendement annuel moyen d\'un portefeuille indiciel. S&P 500 historique ~9 % nominal. On utilise 8 % par prudence. Ajustez avec le slider ci-dessus.', tab: null },
                  { label: `Inflation ${(h.inflationGen * 100).toFixed(1)} %/an`, desc: 'Taux d\'inflation annuel pour convertir les dollars futurs en dollars d\'aujourd\'hui. Cible de la Banque du Canada : 2 %.', tab: mode === 'avance' ? 'params' : null },
                  { label: `Horizon ${h.horizon} ans`, desc: 'Durée de la simulation. Ajustez avec le slider "Horizon d\'analyse" ci-dessus.', tab: null },
                ].map((item, i) => (
                  <span key={i}>
                    {item.tab ? (
                      <Tooltip2 text={`${item.desc} → Cliquer pour ajuster`}>
                        <button
                          onClick={() => setActiveTab(item.tab)}
                          className="inline-flex items-center gap-1 text-xs bg-white border border-stone-300 hover:border-stone-500 hover:bg-stone-100 rounded-full px-3 py-1 text-stone-700 transition-colors cursor-pointer"
                        >
                          {item.label} <span className="text-stone-400">✎</span>
                        </button>
                      </Tooltip2>
                    ) : (
                      <Tooltip2 text={item.desc}>
                        <span className="inline-flex items-center text-xs bg-white border border-stone-200 rounded-full px-3 py-1 text-stone-600">
                          {item.label}
                        </span>
                      </Tooltip2>
                    )}
            ...
            {/* Graphique évolution */}
            <div className="bg-white rounded-lg border border-stone-200 p-5">
              <h3 className="text-lg font-bold text-stone-900 mb-1">Évolution sur {h.horizon} ans</h3>
              <p className="text-xs text-stone-500 mb-1">Valeurs nettes équivalentes (incluant frais de sortie pour l'immo).{dollarsReels ? ' Ajusté pour inflation.' : ''}</p>
              <p className="text-xs text-stone-400 mb-4">Point de départ commun : <span className="font-medium text-stone-600">{fmtMoney(rp.coutInitial)}</span> investis (même capital pour les 4 stratégies)</p>
              <ResponsiveContainer width="100%" height={340}>
                <LineChart data={timeSeriesData} margin={{ bottom: 10 }}>
                  ...
                  <Line type="monotone" dataKey="Multiplex" stroke="#0369a1" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="Je loue mon bien" stroke="#7c2d12" strokeWidth={2} dot={false} />
                  <Line type="monotone" dataKey="Je reste loc. + bourse" stroke="#b45309" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>

            {/* Graphique valeur totale non-liquidée (mode avancé) */}
            {mode === 'avance' && (() => {
              const tsValeurBrute = rp.timeseries.map((r, i) => ({
                annee: r.annee,
                'Résid. princ. (équité)': applyReel(r.equite),
                'Multiplex (valeur bien)': applyReel(duplex.timeseries[i].valeurBien),
                'Je loue mon bien (valeur)': applyReel(loc.timeseries[i].valeurBien),
                'Bourse (rester locataire)': applyReel(bourse.timeseries[i].porteTotal),
              }));
              return (
                <div className="bg-white rounded-lg border border-stone-200 p-5">
                  <h3 className="text-lg font-bold text-stone-900 mb-1">Valeur totale des actifs (avant vente)</h3>
                  <p className="text-xs text-stone-500 mb-4">
                    Valeur brute de chaque actif sans supposer de vente — équité immobilière (valeur bien − solde hypothèque) ou valeur du portefeuille boursier. <strong>Sans déduction des frais de vente ni impôts.</strong>
                    {dollarsReels ? ' Ajusté pour inflation.' : ''}
                  </p>
                  <ResponsiveContainer width="100%" height={300}>
                    <LineChart data={tsValeurBrute}>
                      <CartesianGrid strokeDasharray="3 3" stroke="#e7e5e4" />
                      <XAxis dataKey="annee" stroke="#78716c" tick={{ fontSize: 11 }} />
                      <YAxis stroke="#78716c" tickFormatter={(v) => `${(v / 1000).toFixed(0)}k`} tick={{ fontSize: 11 }} />
                      <Tooltip formatter={(v) => fmtMoney(v)} />
                      <Legend />
                      <Line type="monotone" dataKey="Résid. princ. (équité)" stroke="#059669" strokeWidth={2} dot={false} />
                      <Line type="monotone" dataKey="Multiplex (valeur bien)" stroke="#0369a1" strokeWidth={2} dot={false} />
                      <Line type="monotone" dataKey="Je loue mon bien (valeur)" stroke="#7c2d12" strokeWidth={2} dot={false} />
                      <Line type="monotone" dataKey="Bourse (rester locataire)" stroke="#b45309" strokeWidth={2} dot={false} />
                    </LineChart>
                  </ResponsiveContainer>
                </div>
              );
                ...
                <table className="w-full text-sm" style={{ minWidth: '640px' }}>
                  ...
                  <tbody>
                    ...
                    <tr className="border-t border-stone-200">
                      ...
                      <td className="px-4 py-3 text-stone-600">Faible</td>
                      <td className="px-4 py-3 text-right font-mono font-bold text-stone-800">{fmtMoney(applyReel(loc.beneficeNet))}</td>
                    </tr>
                    <tr className="border-t border-stone-200 bg-stone-50">
                      <td className="px-4 py-3 font-semibold">Je reste locataire + bourse</td>
                      <td className="px-4 py-3 text-stone-600">Vous restez locataire (c'est vous le locataire) et placez l'équivalent de la mise de fonds + économies annuelles en bourse (CELI, REER, non-enr.).</td>
        ...
        {activeTab === 'loc' && (() => {
          ...
          const loyerMensuelBrut = h.loyerPercuInitial * (1 - h.vacancePct) * (1 - h.gestionPct);
          return (
          <div className="space-y-6">
            <details className="bg-stone-50 border border-stone-200 rounded-lg overflow-hidden">
              <summary className="px-4 py-3 cursor-pointer text-sm font-semibold text-stone-700 hover:bg-stone-100 transition-colors flex items-center justify-between">
              ...
              <div className="px-4 pb-4 pt-2 text-xs text-stone-600 space-y-2 border-t border-stone-200">
                ...
                <p><strong>Coût de logement à garder en tête :</strong> pendant {h.horizon} ans, vous payez un loyer en parallèle — soit <strong>{fmtMoney(loc.loyerPayeCum)}</strong> cumulé (nominal). Ce montant est affiché comme indicateur séparé. Dans le scénario Bourse, le loyer est intégré différemment (il réduit les montants investis chaque année), donc les bénéfices nets restent comparables entre stratégies.</p>
                <p><strong>Différence clé vs Multiplex :</strong> vous ne profitez pas de l'exemption résidence principale — 50 % du gain en capital réalisé à la vente est imposé à votre TMI ({(meta.tmiActuel * 100).toFixed(1)} %). Cela représente un coût fiscal important à la sortie.</p>
                <p><strong>La mise de fonds minimale</strong> pour un locatif pur est de 20 % (pas d'assurance SCHL possible — non disponible pour les immeubles locatifs). Le taux hypothécaire est généralement +0,5 % supérieur à celui d'une résidence principale.</p>
              </div>
            </details>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
              <MoneyCard label="Versement mensuel" value={loc.versementMens} color="stone" tooltip="Paiement hypothécaire mensuel (capital + intérêts). Pour un bien locatif pur, le taux est généralement +0,5 % vs résidence principale et la mise de fonds minimale est 20 %." />
              <MoneyCard label="Loyer payé (vous, cumulé)" value={-applyReel(loc.loyerPayeCum)} color="red" tooltip={`Pendant ${h.horizon} ans, vous habitez comme locataire et payez ${fmtMoney(h.loyerInitial)}/mois (initial, indexé à ${(h.augmLoyer * 100).toFixed(1)} %/an). Ce montant est informatif — à comparer au coût de votre logement dans les autres scénarios (hypothèque + charges dans RP/Multiplex).`} />
              <MoneyCard label="Impôt sur gain en capital" value={-applyReel(loc.impotGainCap)} color="red" tooltip="À la vente d'un immeuble locatif, 50 % du gain en capital est imposé à votre TMI. Ex : gain de 300 000 $ → 150 000 $ ajoutés à votre revenu → ~55 000 $ d'impôt à 37 % de TMI. La résidence principale est exemptée de cet impôt." />
              <MoneyCard label="Bénéfice net (après vente)" value={applyReel(loc.beneficeNet)} color={loc.beneficeNet >= 0 ? 'green' : 'red'} emphasis sublabel={dollarsReels ? "en $ d'aujourd'hui · après impôts" : 'après impôts'} tooltip={`Simulation sur ${h.horizon} ans avec vente du bien à la fin. Produit de vente − solde hypothèque − commissions − impôt sur gain en capital (50 % inclusion) − coût initial + cashflow cumulé − loyer payé comme locataire (${fmtMoney(loc.loyerPayeCum)} cumulé).`} />
            </div>
        ...
        {activeTab === 'bourse' && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
              ...
              <MoneyCard label="Bénéfice net" value={applyReel(bourse.beneficeNet)} color="green" emphasis tooltip={`Simulation sur ${h.horizon} ans. Bénéfice net = CELI + REER net d'impôt (×(1−TMI retraite)) + Non-enr. − capital total investi. Le loyer payé est intégré : chaque année, le montant investi = max(0, coût de propriété − loyer), donc un loyer élevé réduit directement le portefeuille final.`} />
            </div>
            <div className="bg-white rounded-lg border border-stone-200 p-6">
        ...
        {activeTab === 'retraite' && (
          <div className="space-y-6">
            ...
            <>
            <div className="bg-white rounded-lg border border-stone-200 p-6">
              <h3 className="text-lg font-bold text-stone-900 mb-1">Décaissement à la retraite</h3>
        ...
        {activeTab === 'params' && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <section className="bg-white rounded-lg border border-stone-200 p-5">
              ...
              <Slider label="Rendement bourse annuel" value={h.rendement} onChange={update('rendement')} min={0.03} max={0.12} step={0.0025} format="pct" sourceKey="sp500" help="S&P 500 historique ~9 % nominal. On utilise 8 % par prudence." />
        ...
        <div className="fixed inset-0 bg-stone-900/80 z-50 flex items-center justify-center p-4">
          <div className="bg-white max-w-md w-full rounded-xl shadow-2xl overflow-hidden">
            <div className="bg-stone-900 text-white px-6 py-5">
              <div className="text-xs uppercase tracking-widest text-stone-400 mb-1">Simulateur · Île de Montréal · 2026</div>
              <h2 className="text-xl font-bold">Où investir mes économies ?</h2>
              <p className="text-sm text-stone-400 mt-2 leading-relaxed">
                Ce simulateur compare 4 stratégies d'investissement sur l'Île de Montréal :
                acheter une résidence principale, un duplex, un immeuble locatif, ou rester locataire et placer en bourse.
                Les calculs incluent la fiscalité réelle du Québec (TMI, CELI, REER) et les données de marché officielles.
              </p>
            </div>
            <div className="px-6 py-5 space-y-4">
              <p className="text-sm text-stone-600 font-medium">Personnalisez en 30 secondes :</p>
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">J'ai <span className="text-stone-900 font-bold">{rawH.ageActuel}</span> ans</label>
                <input
                  type="range" min={18} max={65} step={1} value={rawH.ageActuel}
                  onChange={(e) => setRawH(prev => ({ ...prev, ageActuel: parseInt(e.target.value) }))}
                  className="w-full h-1.5 bg-stone-200 rounded-lg appearance-none cursor-pointer accent-stone-800"
                />
                <div className="flex justify-between text-xs text-stone-400 mt-0.5"><span>18 ans</span><span>65 ans</span></div>
              </div>
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">
                  J'ai <span className="text-stone-900 font-bold">{rawH.prix ? `${Math.round(rawH.prix * rawH.miseFondsPct).toLocaleString('fr-CA')} $` : '—'}</span> à investir
                  <span className="text-stone-400 font-normal"> — en immobilier (mise de fonds) ou en bourse</span>
                </label>
                <input
                  type="range" min={20000} max={200000} step={5000} value={Math.round(rawH.prix * rawH.miseFondsPct)}
                  onChange={(e) => {
                    const apport = parseInt(e.target.value);
                    setRawH(prev => ({ ...prev, prix: Math.round(apport / prev.miseFondsPct) }));
                  }}
                  className="w-full h-1.5 bg-stone-200 rounded-lg appearance-none cursor-pointer accent-stone-800"
                />
                <div className="flex justify-between text-xs text-stone-400 mt-0.5"><span>20 000 $</span><span>200 000 $</span></div>
              </div>
              <div>
                <label className="block text-sm font-medium text-stone-700 mb-1">
                  Au Canada depuis <span className="text-stone-900 font-bold">{rawH.anneeArriveeCanada}</span>
                  <span className="text-stone-400 font-normal"> — requis pour calculer vos droits CELI cumulatifs</span>
                </label>
                <input
                  type="range" min={2009} max={2026} step={1} value={rawH.anneeArriveeCanada}
                  onChange={(e) => setRawH(prev => ({ ...prev, anneeArriveeCanada: parseInt(e.target.value) }))}
                  className="w-full h-1.5 bg-stone-200 rounded-lg appearance-none cursor-pointer accent-stone-800"
                />
                <div className="flex justify-between text-xs text-stone-400 mt-0.5">
                  <span>2009 ou avant</span>
                  <span>Droits CELI : <span className="font-semibold text-stone-600">{fmtMoney(droitsCeliCumulatifs(rawH.anneeArriveeCanada))}</span></span>
                  <span>2026</span>
                </div>
              </div>
              <p className="text-xs text-stone-400 border-t border-stone-100 pt-3">
                🔒 Aucune information personnelle n'est collectée ou transmise. Toutes vos données restent dans votre navigateur.
              </p>
              <button
                onClick={() => {
                  localStorage.setItem('imtl_seen', '1');
                  setShowOnboarding(false);
                  setActiveTab('comparison');
                }}
                className="w-full bg-stone-900 text-white py-3 rounded-lg font-medium hover:bg-stone-800 transition-colors mt-2"
              >
                Voir mes résultats →
              </button>
              <button
              ...
              >
                Explorer d'abord sans personnaliser
              </button>
            </div>
          </div>
        </div>
      ...
      <footer className="border-t border-stone-200 bg-stone-50 mt-12">
        <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 space-y-4">
          {/* Sources et méthodologie — en haut */}
          <details className="bg-white border border-stone-200 rounded-lg overflow-hidden text-xs">
            <summary className="px-4 py-3 cursor-pointer font-semibold text-stone-700 hover:bg-stone-50 transition-colors flex items-center justify-between">
